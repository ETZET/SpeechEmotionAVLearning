...
class UMAP(BaseEstimator):
    ...
    def set_custom_intialization(self, X, label, anchors_mapping):
        """
        X: nd.array (num_samples x features)
        label: nd.array (num_samples)
        anchors_mapping: List of reference Arousal-Valence values ordered by the class number
        :return 2d numpy array that stores the the intialized positions 
        """
        # getting the average vector per class
        ...
        random_normal = random_state.normal(
            loc=0.0, scale=0.0, size=list(init.shape)
        ).astype(np.float32)
