...
class UMAP(BaseEstimator):
    ...
    def fit(self, X, y=None, force_all_finite=True):
        ...
        # normalize embedding to (-1 and 1) along each dimension and keep track of parameters
        self.scaler = MinMaxScaler(feature_range=(-1, 1))
        self.embedding_ = self.scaler.fit_transform(self.embedding_)
    ...
    def transform(self, X, label, anchors, force_all_finite=True):
        ...
        # hack
        # embedding = self.set_custom_intialization(X, label, anchors_mapping)
        ...
        if self.n_epochs is None:
            ...
            n_epochs = int(self.n_epochs // 10.0)
        ...
        # post-processing - normalize output
    ...
    def set_custom_intialization(self, X, label, anchors_mapping):
        """
        Initializes positions for samples by adding noise to class-specific anchor points.

        Parameters:
        X (numpy.ndarray): 2D array (num_samples, features) representing sample features.
        label (numpy.ndarray): 1D array (num_samples,) with class labels.
        anchors_mapping (list): List of Arousal-Valence arrays for each class.

        Returns:
        numpy.ndarray: 2D array with initialized positions for each sample.

        The positions are computed as each class's anchor point plus random noise.
        """
        num_labels = len(np.unique(label))
        init = np.zeros((X.shape[0], len(anchors_mapping[0])))
